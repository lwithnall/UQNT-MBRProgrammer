...
/* Just convert an XML dom object to a string*/
BlockMirrorTextToBlocks.xmlToString = function (xml) {
    return new XMLSerializer().serializeToString(xml);
};

/* convert code to default Blockly text block */
BlockMirrorTextToBlocks.prototype.convertSourceToCodeBlock = function (python_source) {
    var xml = document.createElement("xml"); // <xml> element
    xml.appendChild(BlockMirrorTextToBlocks.raw_block(python_source)); // default raw (text) block
    return BlockMirrorTextToBlocks.xmlToString(xml); // convert to string
};
...
BlockMirrorTextToBlocks.prototype.convertSource = function (filename, python_source) {
    ...
    this.source = python_source.split("\n"); // split python code into individual lines
    let previousLine = 1+this.source.length;
    let startLine = 1;

    while (ast === null) {
        // NOTHING (LEFT) IN PYTHON SOURCE FILE
        if (python_source.trim() === "") {
            if (badChunks.length) {
                // if bad chunks exist output all of them as default text block
        ...
        // ON ERROR ADD TO BAD CHUNKS AND CONTINUE
        } catch (e) {
            //console.error(e);
            error = e;
...
/* Make a raw block i.e. default text field if code can't be converted */
