...
BlockMirrorTextToBlocks.BINOPS = [
  ...
  [
    '**',
    'Pow',
    python.pythonGenerator.ORDER_EXPONENTIATION,
  ...
  [
    '<<',
    'LShift',
    python.pythonGenerator.ORDER_BITWISE_SHIFT,
  ...
  [
    '|',
    'BitOr',
    python.pythonGenerator.ORDER_BITWISE_OR,
  ...
  [
    '^',
    'BitXor',
    python.pythonGenerator.ORDER_BITWISE_XOR,
  ...
  [
    '&',
    'BitAnd',
    python.pythonGenerator.ORDER_BITWISE_AND,
...
var BINOPS_BLOCKLY_DISPLAY_FULL = BlockMirrorTextToBlocks.BINOPS.map((binop) => [
  binop[0],
  binop[1],
]);
...
BlockMirrorTextToBlocks.BINOPS_AUGASSIGN_DISPLAY =
  BlockMirrorTextToBlocks.BINOPS_AUGASSIGN_DISPLAY_FULL.filter(
    (binop) => BINOPS_SIMPLE.indexOf(binop[1]) >= 0
  );
...
python.pythonGenerator.forBlock['ast_BinOp'] = function (block, generator) {
  ...
  var order = tuple[1];
  var argument0 =
    python.pythonGenerator.valueToCode(block, 'A', order) || python.pythonGenerator.blank;
  var argument1 =
    python.pythonGenerator.valueToCode(block, 'B', order) || python.pythonGenerator.blank;
